<script>
    ...
    let ljudiGet = async function(){
        loading = true
        let [resp, err] = await fetch2('get', 'timesheet_ljudi_by_date_get', {selected_date:getISODatePartFromDate($selectedDate), construction_id:$page.params.construction_id})
        loading = false
        if (resp) {
  ...
  {#if ljudi && ljudi.length}
    ...
    <div class="row mb-3">
        <div class="col">
            ...
            <strong>{(ljudi && ljudi[selected_member_ix]) ? ljudi[selected_member_ix].night_hours : ''}</strong>
        </div>
        <div class="col">
            <div class="text-success">Rad na praznike</div>
            <strong>{(ljudi && ljudi[selected_member_ix]) ? ljudi[selected_member_ix].hollyday_hours : ''}</strong>
        </div>
    </div>
    <hr>
    <div class="row mb-3">
